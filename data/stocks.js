...
const getStockData = async (ticker) => {
    ...
    const quoteSummary2 = await yahooFinance.quoteSummary(ticker, { modules: ['assetProfile'] });
    ...
    if (quote.regularMarketChange !== null && quote.regularMarketChange !== undefined) {
        data.priceChange = "$"+ quote.regularMarketChange.toFixed(2).toString();
    }
    if (quote.percentChange !== null || quote.percentChange !== undefined) {
        data.percentChange = quote.regularMarketChangePercent.toFixed(2) +"%";
    }
    if (quote.regularMarketOpen !== null && quote.regularMarketOpen !== undefined) {
        data.openPrice = "$"+ quote.regularMarketOpen.toString();
    }
    if (quote.regularMarketPreviousClose !== null && quote.regularMarketPreviousClose !== undefined) {
        data.previousClose = "$"+ quote.regularMarketPreviousClose.toString();
    }
    if (quote.regularMarketDayHigh !== null && quote.regularMarketDayHigh !== undefined) {
        data.dayHigh = "$"+ quote.regularMarketDayHigh.toString();
    }
    if (quote.regularMarketDayLow !== null && quote.regularMarketDayLow !== undefined) {
        data.dayLow = "$"+ quote.regularMarketDayLow.toString();
    }
    if (quote.regularMarketVolume === null || quote.regularMarketVolume === undefined) {
        data.volume = "N/A";
    }
    if (quote.marketCap !== null && quote.marketCap !== undefined) {
        ...
        for (const { value, symbol } of suffixes) {
            if (absMarketCap >= value) {
                marketCapAbbrev = "$"+(quote.marketCap / value).toFixed(2).replace(/\.00$/, "") + symbol;
                break;
            }
        }
